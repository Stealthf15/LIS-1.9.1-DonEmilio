...
Public Class frmHemaAddTest
    ...
    Public Sub LoadRecords()
        Try
            ...
            Connect()
            rs.Connection = conn
            rs.CommandText = "SELECT * FROM `default_specimen` WHERE `section` = 'Hematology' AND `status` = 'Enable' ORDER BY `order_no`"
    ...
    Private Sub btnAddTest_ItemClick(ByVal sender As Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnAddTest.ItemClick
        If TypeResult = "New" Then
            For x = Me.lvList.CheckedItems.Count - 1 To 0 Step -1
                ...
                SaveRecordwthoutMSG("INSERT INTO `tmpresult` (`universal_id`, `measurement`,`test_code`, `sample_id`, `date`, `patient_id`, `order_no`, `test_group`, `units`, `unit_conv`, `instrument`, `his_code`, `section`, `sub_section`) VALUES " _
                            & "(" _
