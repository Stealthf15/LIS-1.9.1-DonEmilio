    ...
    Public Sub LoadTest()
        ...
        If _NoOfRerun = "0" Then
            Try
                ...
                WHERE `sample_id` = @MainID AND `section` = @Section AND `status` = '0' ORDER BY `order_no`"
        ...
        ElseIf _NoOfRerun = "1" Then
            Try
                ...
                `ref_conv` AS ConvRefRange,  `instrument` AS Instrument, `test_code` AS TestCode, `id` AS ID, 
                `test_group` AS TestGroup FROM `rerun_result` 
                WHERE `sample_id` = @MainID AND `section` = @Section AND `status` = '1' ORDER BY `order_no`"
        ...
        Else
            Try
                ...
                `ref_conv` AS ConvRefRange,  `instrument` AS Instrument, `test_code` AS TestCode, `id` AS ID, 
                `test_group` AS TestGroup FROM `rerun_result` 
                WHERE `sample_id` = @MainID AND `section` = @Section AND `status` = '" & cboRerun.Text & "' ORDER BY `order_no`"
