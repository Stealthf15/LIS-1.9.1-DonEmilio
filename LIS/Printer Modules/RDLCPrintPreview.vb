    ...
    Public Sub PrintPreview(ByVal SampleID As String, ByVal WorkListTable As String, ByVal ResultTable As String, ByVal PrintStatus As String, ByVal Section As String, ByVal SubSection As String, ByVal Frm As Form, ByVal ReportViewerData As Microsoft.Reporting.WinForms.ReportViewer)
        Try
            ...
            Dim SQL = "SELECT
					...
					DATE_FORMAT(`tmpworklist`.`dt_released`, '%m/%d/%Y %r') AS DateReleased,
				...
				WHERE (`tmpworklist`.`status` = 'Processing' OR `tmpworklist`.`status` = 'Result Received' OR `tmpworklist`.`status` = 'Validated') 
                ...
                AND (`tmpworklist`.`sub_section` = '" & SubSection & "')
				AND (`tmpworklist`.`main_id` = '" & SampleID & "')
            ...
            'Result Table
            Connect()
            Dim result_SQL = "SELECT `tmpresult`.`universal_id` AS TestName, 
						...
						WHERE `tmpresult`.`sample_id` = '" & SampleID & "' AND `tmpresult`.section = '" & Section & "' AND `tmpresult`.sub_section = '" & SubSection & "' AND print_status = '" & PrintStatus & "' GROUP BY `tmpresult`.test_code ORDER BY specimen.order_no ASC;"
